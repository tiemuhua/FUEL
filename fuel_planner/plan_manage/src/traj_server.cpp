...
void replanCallback(std_msgs::Empty msg) {
    ...
    double t_stop = (ros::Time::now() - start_time).toSec() + time_out + replan_time_;
...
void cmdCallback(const ros::TimerEvent &e) {
    ...
    double t_cur = (ros::Time::now() - start_time).toSec();
...
void test() {
    ...
    for (double ts = 0; ts <= duration2; ts += 0.01)
        traj_pts.emplace_back(fitted.evaluateDeBoorT(ts));
