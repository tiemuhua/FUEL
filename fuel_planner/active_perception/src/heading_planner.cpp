...
// template <typename VERTEX>
    ...
    HeadingPlanner::HeadingPlanner(ros::NodeHandle &nh, const shared_ptr<SDFMap> &map) {
        ...
        for (size_t i = 0; i < 2 * half_vert_num_ + 1; ++i) {
    ...
    void HeadingPlanner::searchPathOfYaw(const vector<Eigen::Vector3d> &pts, const vector<double> &yaws,
        ...
        for (size_t i = 0; i < yaws.size(); ++i) {
            ...
            if (start_end) {  // start and end vertice
                ...
                for (size_t j = 0; j < vert_num; ++j) {  // evaluate info gain in parallel
