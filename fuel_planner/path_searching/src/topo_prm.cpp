...
namespace fast_planner {
    TopologyPRM::TopologyPRM(ros::NodeHandle &nh, const EDTEnvironment::Ptr &env) {
        graph_.clear();
        ...
        for (size_t i = 0; i < max_raw_path_; ++i) {
            casters_[i] = std::make_unique<RayCaster>();
        }
    }
    ...
    list<GraphNode::Ptr> TopologyPRM::createGraph(const Eigen::Vector3d& start, const Eigen::Vector3d& end) {
    ...
    vector<GraphNode::Ptr> TopologyPRM::findVisibGuard(const Eigen::Vector3d& pt) {
        ...
        if (graph_.size() > 2) {
            for (auto iter1 = graph_.begin(); iter1 != graph_.end() && graph_.size() > 2; ++iter1) {
                ...
                if ((*iter1)->neighbors_.size() <= 1) {
                    ...
                    for (auto & iter2 : graph_) {
                        for (auto it_nb = iter2->neighbors_.begin();
    ...
    vector<vector<Eigen::Vector3d>> TopologyPRM::pruneEquivalent(vector<vector<Eigen::Vector3d>> &paths) {
        ...
        if (paths.empty()) return pruned_paths;
        ...
        for (int i = 1; i < paths.size(); ++i) {
            ...
            for (int j : exist_paths_id) {
                // compare with one path
                bool same_topo = sameTopoPath(paths[i], paths[j], 0.0);
        ...
        // save pruned paths
        for (int i : exist_paths_id) {
            pruned_paths.push_back(paths[i]);
        }
    ...
    vector<vector<Eigen::Vector3d>> TopologyPRM::selectShortPaths(vector<vector<Eigen::Vector3d>> &paths,
        ...
        for (size_t i = 0; i < reserve_num_ && !paths.empty(); ++i) {
        ...
        for (auto & short_path : short_paths) {
            short_path.insert(short_path.begin(), start_pts_.begin(), start_pts_.end());
            short_path.insert(short_path.end(), end_pts_.begin(), end_pts_.end());
        }
    ...
    void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
        vector<Eigen::Vector3d> short_path = std::move(path);
    ...
    void TopologyPRM::shortcutPaths() {
        ...
        if (parallel_shortcut_) {
            ...
            for (size_t i = 0; i < raw_paths_.size(); ++i) {
                short_threads.emplace_back(thread(&TopologyPRM::shortcutPath, this, raw_paths_[i], i, 1));
            }
            for (size_t i = 0; i < raw_paths_.size(); ++i) {
    ...
    vector<Eigen::Vector3d> TopologyPRM::discretizeLine(Eigen::Vector3d p1, Eigen::Vector3d p2) {
        ...
        for (size_t i = 0; i <= seg_num; ++i)
            line_pts.emplace_back(p1 + dir * double(i) / double(seg_num));
    ...
    vector<vector<Eigen::Vector3d>> TopologyPRM::discretizePaths(vector<vector<Eigen::Vector3d>> &path) {
        ...
        for (auto & i : path) {
            dis_path = discretizePath(i);

            if (!dis_path.empty()) dis_paths.push_back(dis_path);
        }
...
// search for useful path in the topo graph by DFS
    vector<vector<Eigen::Vector3d>> TopologyPRM::searchPaths() {
        ...
        for (int i = min_node_num; i <= max_node_num; ++i) {
            ...
            for (int j : path_list[i]) {
                filter_raw_paths.push_back(raw_paths_[j]);
    ...
    void TopologyPRM::depthFirstSearch(vector<GraphNode::Ptr> &vis) {
        ...
        for (size_t i = 0; i < cur->neighbors_.size(); ++i) {
            ...
            bool revisit = false;
            for (auto & vi : vis) {
                if (cur->neighbors_[i]->id_ == vi->id_) {
                    revisit = true;
                    break;
                }
            }
