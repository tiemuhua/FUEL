...
/* ---------- node of topo graph ---------- */
    ...
    private:
        ...
        list<GraphNode::Ptr> createGraph(const Eigen::Vector3d &start, const Eigen::Vector3d &end);
        ...
        vector<GraphNode::Ptr> findVisibGuard(const Eigen::Vector3d &pt);  // find pairs of visibile
        // guard
        ...
        static vector<Eigen::Vector3d> discretizePath(const vector<Eigen::Vector3d> &path, int pt_num);

        bool sameTopoPath(const vector<Eigen::Vector3d> &path1, const vector<Eigen::Vector3d> &path2,
                          double thresh);

        static Eigen::Vector3d getOrthoPoint(const vector<Eigen::Vector3d> &path);

        static int shortestPath(vector<vector<Eigen::Vector3d>> &paths);
...
//  void setEnvironment(const EDTEnvironment::Ptr& env);
        ...
        TopologyPRM(ros::NodeHandle &nh, const EDTEnvironment::Ptr &env);

        void findTopoPaths(const Eigen::Vector3d &start, const Eigen::Vector3d &end, vector<Eigen::Vector3d> start_pts,
