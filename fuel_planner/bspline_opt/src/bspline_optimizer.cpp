...
namespace fast_planner {
    ...
    BsplineOptimizer::BsplineOptimizer(ros::NodeHandle &nh, const EDTEnvironment::Ptr &env) {
    ...
    void BsplineOptimizer::optimize(Eigen::MatrixXd &points, double &dt, const int &cost_function,
        ...
        pt_dist_ = 0.0;
        for (size_t i = 0; i < control_points_.rows() - 1; ++i) {
 ...
 * guiding point on the path and the distance between them is penalized */
    ...
    void BsplineOptimizer::calcTimeCost(const double &dt, double &cost, double &gt) const {
        // Min time
