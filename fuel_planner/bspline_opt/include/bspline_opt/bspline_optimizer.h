...
//        vector<Eigen::Vector3d> matrixToVectors(const Eigen::MatrixXd &ctrl_pts);
    ...
    private:
        ...
        void calcSmoothnessCost(const vector<Eigen::Vector3d> &q, const double &dt, double &cost,
                                vector<Eigen::Vector3d> &gradient_q, double &gt) const;
        ...
        void calcTimeCost(const double &dt, double &cost, double &gt) const;
    ...
    public:
        ...
        void getCostCurve(vector<double> &cost, vector<double> &time) const {
            cost = vec_cost_;
            time = vec_time_;
        }
